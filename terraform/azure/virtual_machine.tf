...
# Create the network interfaces and public IP addresses for the nodes.
...
resource "azurerm_network_interface" "node_002" {
  name                = "nic-vm-node-${local.suffix}-002"
  location            = var.location
  resource_group_name = azurerm_resource_group.default.name
  ...
  ip_configuration {
    ...
    subnet_id                     = azurerm_subnet.default.id
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = azurerm_public_ip.node_002.id
  }
}

resource "azurerm_public_ip" "node_002" {
...
resource "azurerm_linux_virtual_machine" "node_002" {
  name                = "vm-node-${local.suffix}-002"
  location            = var.location
  resource_group_name = azurerm_resource_group.default.name
  computer_name       = "node-002"
  size                = "Standard_D2s_v5"
  admin_username      = "adminuser"
  secure_boot_enabled = true

  network_interface_ids = [
    azurerm_network_interface.node_002.id,
  ]
  ...
  os_disk {
    name                   = "osdisk-vm-node-${local.suffix}-002"
    caching                = "ReadWrite"
    storage_account_type   = "Standard_LRS"
    disk_encryption_set_id = azurerm_disk_encryption_set.default.id
  }
